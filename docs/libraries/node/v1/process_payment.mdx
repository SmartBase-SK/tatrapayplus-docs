...
This method works like the others by calling the API and returning data based on the API reference.
Additionally, it includes a `simpleStatus` attribute that can have one of three values: `ACCEPTED`, `PENDING`, or `REJECTED`.
...
More details on this feature can be found [💳 here 💳](/docs/libraries/node/v1/save_card).
...
Using the saved `paymentId` with the `getPaymentStatus` method, you can obtain the current status and take the appropriate action in your system.
